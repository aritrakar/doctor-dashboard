...
import "../Doctor/DocPatients.css";
import Button from "@material-ui/core/Button";
import { DataGrid } from "@material-ui/data-grid";

export default class PatRow extends React.Component {
  render() {
    ...
    var columns = items.map((item, i) => {
      if (item === "when1" && items.includes("last_consul")) {
        return {
          ...
          width: 300,
          valueGetter: (params) => {
            var d = params.row.last_consul?.seconds;
        ...
        return {
          ...
          width: 300,
          valueGetter: (params) => {
            var d = params.row.next_consul?.seconds;
            var n = new Date(d * 1000);
            var res = n.toString();
            return res.split("GMT")[0];
          },
    ...
    columns = columns.filter((item) => {
      return item.field !== "last_consul" && item.field !== "next_consul";
    });
